...
import { soundEnabled, setSoundEnabled, playMoveSound } from './sound.js'
...
export function SettingsPanel() {
  ...
  function toggleSound() {
    ...
    setSound(nv)
    setSoundEnabled(nv)
    if (nv) playMoveSound('move') // sample + unlocks audio on this user gesture
  }
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              background: sound ? 'var(--accent)' : 'var(--surface-2)',
              color: sound ? '#fff' : 'var(--text)',
              border: '1px solid var(--border)', borderRadius: 7, cursor: 'pointer',
            }}
          >
            {sound ? '🔊 Move sounds on' : '🔇 Move sounds off'}
          </button>
        </div>
      )}
    </div>
