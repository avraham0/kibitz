...
type CritPos = { ply: number; san: string; bestSan: string; fenBefore: string; cpLoss: number; type: MistakeType; missed: boolean }

function analysisLink(fen: string) {
  return `https://www.chess.com/analysis?fen=${encodeURIComponent(fen)}`
}

export function CriticalPositions({ games }: { games: GameSummary[] }) {
  ...
  for (const g of games) {
    for (const m of g.moves) {
      ...
      positions.push({ ply: m.ply, san: m.san, bestSan: m.bestSan, fenBefore: m.fenBefore, cpLoss: m.cpLoss, type: m.type, missed: m.missed })
    }
  }
        ...
        {positions.slice(0, 12).map((p, i) => {
          ...
          return (
            <div key={i} style={{ width: 220 }}>
              <Chessboard position={p.fenBefore} boardOrientation={orientationFromFen(p.fenBefore)} customArrows={arrows} arePiecesDraggable={false} boardWidth={220} />
