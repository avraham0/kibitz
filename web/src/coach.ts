import type { Stats, Suggestion, TimeBucket, MistakeType, Phase, CoachableType as CoachableMistakeType } from './api-types.js'

const TYPE_LABEL: Record<CoachableMistakeType, string> = {
  hung_piece: 'Hung pieces', missed_tactic: 'Missed tactics', bad_trade: 'Bad trades',
  king_safety: 'King safety', positional: 'Positional errors', fork: 'Forks',
  pin: 'Pins', skewer: 'Skewers', discovered_attack: 'Discovered attacks',
  trapped_piece: 'Trapped pieces', back_rank: 'Back-rank tactics',
}
...
function examplesFor(blunders: Stats['topBlunders'], type?: MistakeType): Suggestion['examples'] {
...
export function coach(stats: Stats): Suggestion[] {
  ...
  for (const t of Object.keys(stats.byType) as CoachableMistakeType[]) {
    ...
    if (share >= 0.3) {
      const split = (missed > 0 || allowed > 0) ? ` (${missed} you missed, ${allowed} you allowed)` : ''
      out.push({
